...
// --- Aiden's Magic Block World: fire magic, life, and a day/night cycle ---
const creatures = new Creatures(scene, world, particles);
const magic = new MagicSystem(scene, world, player, particles, creatures);
const monsters = new Monsters(scene, world, particles, creatures);
const dayNight = new DayNightCycle(scene, ambientLight, sunLight, cloudMat);
magic.monsters = monsters;
...
ui.dayNight = dayNight;
ui.monsters = monsters;
magic.ui = ui;

monsters.onSpawnFirst = () => {
  ui.showNotification('👾 怪物出现了！用 R 火焰魔法打它们（打两下就没了）');
};
monsters.onKill = (total) => {
  ui.showNotification(`⚔️ 打败怪物！掉落了 🍖 烤肉 — 累计 ${total} 只`);
};
monsters.onHurtPlayer = (dmg) => {
  player.health -= dmg;
  particles.spawnFlameBurst(player.position.x, player.position.y + 0.9, player.position.z, 4, 0.4);
  ui.showNotification('💥 被怪物打到了！吃东西可以回血 (V)', 'error');
  if (player.health <= 0) respawnPlayer('你被怪物打倒了！已送回出生点，生命值回满');
};
...
creatures.onBerry = () => {
  ui.addItem('berry', 1);
  ui.showNotification('🍓 摘到浆果！按 V 可以吃掉回血（小鸡会跟着有浆果的人走）');
};
...
function repopulateGroundLife() {
  return creatures.spawnGroundLifeInWorld(40, 14);
}

repopulateFish();
repopulateGroundLife();

// Auto-load saved world if present
const autoLoad = storage.loadFromLocalStorage();
if (autoLoad.success) {
  // The world was rebuilt from the save — put the fish and ground life back
  repopulateFish();
  repopulateGroundLife();
...
document.getElementById('btn-reset-world')?.addEventListener('click', () => {
  if (confirm('确定要清空所有建筑并重新生成世界吗？')) {
    ...
    const ground = repopulateGroundLife();
    const sy = world.getSurfaceHeight(0, 0) + 2;
    player.position.set(0, sy, 0);
    ui.showNotification(
      `新世界生成完成！🐟 ${fishCount} 条鱼 · 🍓 ${ground.bushes} 丛浆果 · 🐔 ${ground.chickens} 只小鸡`
    );
    sounds.playClickSound();
  }
});
...
window.__game = { scene, world, player, particles, magic, creatures, monsters, dayNight, ui, storage };

// Tell the player when the sky flips, so the change never goes unnoticed
let wasNight = dayNight.isNight();

function announceDayNightFlip() {
  ...
  ui.showNotification(
    night
      ? '🌙 太阳落山了，月亮升起来 — 怪物要出来了，用 R 火焰魔法打它们！'
      : '☀️ 太阳升起来了！怪物会被阳光烧掉，🐟 鱼和 🐔 小鸡照常活着'
  );
}
...
function updateFireDamage(delta) {
  ...
  if (player.health <= 0) {
    respawnPlayer('你被自己的火焰烧到了！已送回出生点，生命值已回满');
  }
}
...
function animate() {
  ...
  announceDayNightFlip();
  magic.update(delta);
  creatures.update(delta, player, ui.items.berry > 0);
  monsters.update(delta, player, dayNight.isDeepNight());
